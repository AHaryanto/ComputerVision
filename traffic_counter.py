...
class TrafficCounter(object):
    def __init__(self, video_source=0,
        ...
        # PLACEHOLDER, user creates it by clicking on several points
        self.black_mask = None

        # this will contain the coordinates of the centers in the previos
        self.prev_centroids = []
    ...
    def _set_up_line(self, line_direction, line_position):
        if line_direction.upper() == 'H' or line_direction is None:
            ...
            raise ValueError(
                'Expected an "H" or a "V" only for line direction')
    ...
    def _draw_bounding_boxes(self, frame, contour_id, bounding_points, cx, cy,
                             prev_cx, prev_cy):
        ...
        cv2.putText(frame, str(contour_id), (cx, cy-15),
                    self.font, 0.4, (255, 0, 0), 2)
    ...
    def _is_line_crossed(self, frame, cx, cy, prev_cx, prev_cy):
        ...
        if self.line_direction.upper() == 'H':
            if (prev_cy <= self.p1_count_line[1] <= cy) or (
                ...
                self.counter += 1
                cv2.line(frame, self.p1_count_line,
                         self.p2_count_line, (0, 255, 0), 5)
                is_crossed = True
    ...
    def bind_objects(self, frame, thresh_img):
        ...
        thresh_img: numpy image after subtracting the background and all
        thresholds and noise reduction operations are applied
        '''
        # this line is for opencv 2.4, and also now for OpenCV 4.4,
        ...
        for c in cnts:
            ...
            # This is the way to do it in opencv 3.1
            ...
            if len(self.prev_centroids) == 0:
                ...
                for i in range(len(self.prev_centroids)):
                    # numpy's way to find the euclidean distance between two
                    # points
            ...
            _is_crossed = self._is_line_crossed(
                frame, cx, cy, prev_cx, prev_cy)
            if _is_crossed:
                print(f"Total Count: {self.counter}")
            self._draw_bounding_boxes(
                frame, cnt_id, points, cx, cy, prev_cx, prev_cy)

            cnt_id += 1
        # updating centroids for next frame
    ...
    def _set_up_masks(self):
        ...
        self.raw_avg = cv2.resize(
            self.raw_avg, (self._vid_width, self._vid_height))

    def make_collage_of_four(self, up_left_img, up_right_img, down_left_img,
                             down_right_img):
        ...
        self.collage_frame[middle_height:total_height,
                           middle_width:total_width] = down_r
        ...
        while True:
            ...
            if frame_id < self.starting_frame:
                cv2.accumulateWeighted(
                    working_img, self.raw_avg, rate_of_influence)
                continue
            ...
            # subtracted_img is the frame after the background has been
            # subtracted from it
            ...
            cv2.line(img, self.p1_count_line, self.p2_count_line,
                     (0, 0, 255), 1)  # counting line
            ...
            self.make_collage_of_four(
                subtracted_img, background_avg, dilated_img, img)
            cv2.putText(self.collage_frame, f"Frame: {frame_id}", (
                15, self.collage_height-15), self.font, 1, (255, 255, 255), 3)
            ...
            if k == 27 or k == ord('q') or k == ord('Q'):
                break
            # if the letter s/S is pressed, a screenshot of the current frame
            # on each window will be saved to the current folder
            elif k == ord('s') or k == ord('S'):
                cv2.imwrite(os.path.join(self.screenshot_folder,
                            f"{frame_id}_masked_frame.jpeg"), working_img)
                cv2.imwrite(os.path.join(self.screenshot_folder,
                            f"{frame_id}_background_subtracted.jpeg"),
                            subtracted_img)
                cv2.imwrite(os.path.join(self.screenshot_folder,
                            f"{frame_id}_threshold_applied.jpeg"), dilated_img)
                cv2.imwrite(os.path.join(self.screenshot_folder,
                ...
                cv2.imwrite(os.path.join(self.screenshot_folder,
                            f"{frame_id}_collage.jpeg"), self.collage_frame)
            if k == ord(' '):  # if spacebar is pressed
                # program is paused for a while
